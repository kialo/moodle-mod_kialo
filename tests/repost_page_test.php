...
final class repost_page_test extends \advanced_testcase {
    ...
    /**
     * The repost must honour the original request method (a POST launch is reposted as a POST).
     *
     * @return void
     * @covers \mod_kialo\output\repost_page
     */
    public function test_post_method_is_honoured(): void {
        $this->resetAfterTest();
        global $PAGE;
        $renderer = $PAGE->get_renderer('mod_kialo');

        $html = $renderer->render(new repost_page(
            'https://moodle.example.com/mod/kialo/lti_auth.php?nonce=abc',
            'post',
            ['lti_message_hint' => 'JWT123']
        ));
