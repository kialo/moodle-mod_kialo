...
class repost_page implements renderable, templatable {
     ...
     *
     * @param string $url The URL to repost to. Must include the original query string so that the LTI
     *                     parameters (nonce, state, lti_message_hint, ...) are preserved.
     * @param array $post Any POST parameters of the original request, to be resubmitted as hidden fields.
     */
    public function __construct(string $url, array $post) {
        $this->url = $url;
        $this->params = array_map(function ($key) use ($post) {
            return ["key" => $key, "value" => $post[$key]];
        }, array_keys($post));
    }
